...
* [ ] **1.1.** Initialize the NestJS project using `npx` with strict TypeScript mode: `npx @nestjs/cli new usdc-tracker --strict`
* [ ] **1.2.** Install production dependencies (`@nestjs/config`, `class-validator`, `class-transformer`, `ethers`, `viem`).
* [ ] **1.3.** Create the `.env` template and configuration file (`RPC_URL`, `BLOCKCHAIN_PROVIDER_TYPE`, `PORT`).
* [ ] **1.4.** Setup and initialize `ConfigModule` inside `AppModule`.

### Step 2: Blockchain Layer (Abstraction & Providers)

* [ ] **2.1.** Define global constants (USDC Smart Contract address, `Transfer` event topic hash).
* [ ] **2.2.** Create the `BlockchainProvider` interface and the unified `UsdcTransferRaw` output data model.
* [ ] **2.3.** Implement `EthersProvider` (utilizing `ethers.JsonRpcProvider` and `eth_getLogs`).
* [ ] **2.4.** Implement `ViemProvider` (utilizing `createPublicClient` and `getLogs`).
* [ ] **2.5.** Create the `BlockchainFactory` to switch implementations dynamically based on the `.env` variable.
* [ ] **2.6.** Encapsulate everything inside `BlockchainModule` and register the custom DI token (`BLOCKCHAIN_PROVIDER_TOKEN`).
...
We have established the foundational structure up to **2.2**. Which task on the checklist should we tackle next? Would you like to implement the `EthersProvider` logic, or should we build the factory and module setup first?